...
def get_localisation_of_annotation(annotation : Annotation) -> tuple[int, int, int, int]: # (In Cytomine, geometries are referenced using a cartesian coordinate system ! https://github.com/cytomine/Cytomine-python-client/blob/master/examples/get_annotations.py)
...
def get_random_shift(random_shift : int, random_state : int) -> np.ndarray:
    ...
    if random_shift > 0:
        shift = np.random.randint(0, (np.abs(random_shift)) * 2, size = (2,)) - random_shift # random int in [-random_shift, random_shift]
    else:
        shift = np.array([0, 0])
...
def download_images(config : dict, root : str):
    ...
    for i, img in tqdm(enumerate(img_collections), total = len(img_collections)):
        ...
        for a, annotation in enumerate(annotations):
            ...
            else: # SAM input size is 1024x1024
